...
#ifdef _MSC_VER
// MSVC编译器使用中文文案
#define TEXT_WINDOW_TITLE       "EGE网格 By wysaid - 2025"
...
#else
// 非MSVC编译器使用英文文案
#define TEXT_WINDOW_TITLE       "EGE Mesh by wysaid - 2025"
...
PIMAGE loadTexture(const char* filename)
{
    ...
    setcolor(RED);
    outtextxy(100, 100, TEXT_LOAD_IMAGE_PROMPT);
    ...
    do
    {
        ...
        if(*buffer == '\0')
            readFileNameDlg(buffer, TEXT_CHOOSE_IMAGE_TITLE);
...
int main(int argv, char** argc)
{
    ...
    sprintf(buffer, TEXT_SHOW_MSG_RULE, net.getIntensity());
